      ...
      <div className="raid-dashboard-shell relative z-10 min-h-screen lg:grid lg:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <div className="min-w-0">
          ...
          <section className={`mx-auto min-w-0 max-w-7xl px-4 sm:px-6 lg:px-8 ${compact ? "py-4" : "py-10"}`}>
            {!compact ? (
              <div className="raid-page-hero-card mb-8 max-w-4xl rounded-[24px] border border-relic/20 bg-[#04070d]/88 p-5 shadow-[0_22px_70px_rgba(0,0,0,0.62)] backdrop-blur-md sm:p-6">
                <p className="text-sm font-semibold tracking-[0.14em] text-relic">{resolveText(eyebrow, language)}</p>
                <h1 className="mt-3 break-words font-[var(--font-display)] text-3xl font-light leading-tight text-white sm:text-5xl">{resolveText(title, language)}</h1>
                {resolvedDescription ? <p className="mt-4 text-base font-medium leading-8 text-zinc-100">{resolvedDescription}</p> : null}
              </div>
            ) : null}
            {children}
